...
class SabDabDataset(torch.utils.data.Dataset):
    ...
    def preprocess(self, file_path, save_dir, num_entry_per_file):
        ...
        for line in tqdm(lines):
            ...
            try:
                complex = SabDabComplex.from_pdb(
...
class DBDataset(torch.utils.data.Dataset):
    ...
    def preprocess(self, file_path, save_dir, num_entry_per_file):
        ...
        num_entry_per_file.
        '''
        base_path = os.path.join(os.path.dirname(file_path), 'structures')
        with open(file_path, 'r') as fin:
            lines = fin.read().strip().split('\n')
        for line in tqdm(lines):
            pdb_name = str(line)
            try:
                complex = DBComplex.from_pdb(base_path, pdb_name)
            except Exception as e:
                print_log(f'parse {pdb_name} pdb failed: {e}, skip', level='ERROR')
                continue
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            data = {
                'S': np.array(receptor_seq + ligand_seq),
                'X': np.concatenate((receptor_bb_coord, ligand_bb_coord), axis=0),
                'RP': np.array(receptor_rp + ligand_rp),
                'ID': np.array(receptor_id + ligand_id),
                ### segment, 0 for receptor and 1 for ligand
                'Seg': np.array([0 for _ in range(len(receptor_seq))] + [1 for _ in range(len(ligand_seq))]),
                'center': center,
                'keypoints': keypoints,
            }
        except Exception as e:
    ...
    def preprocess(self, file_path, save_dir, num_entry_per_file):
        ...
        for line in tqdm(lines):
            ...
            try:
                ...
                print_log(f'parse {dill_name} pdb failed: {e}, skip', level='ERROR')
                continue
